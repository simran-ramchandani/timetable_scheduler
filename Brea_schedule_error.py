...
class TimetableScheduler:
    def __init__(self, root):
        ...
        self.classrooms = []
        self.schedule = []
        self.unscheduled_lectures = []  # Track lectures that couldn't be scheduled
        self.scheduling_stats = {}  # Track statistics
    ...
    def setup_ui(self):
        ...
        tk.Label(self.entity_frame, text="Select:", bg="white", fg="black",
        ...
        tk.Label(find_frame, text="Day:", bg="white", fg="#2c3e50", font=("Arial", 8)).pack(anchor=tk.W, pady=2)
        ...
        tk.Label(find_frame, text="Time:", bg="white",fg="#2c3e50", font=("Arial", 8)).pack(anchor=tk.W, pady=2)
    ...
    def generate_schedule(self):
        ...
        try:
            ...
            if success:
                ...
                self.calculate_scheduling_stats(lecture_requirements, assignments)
                ...
                self.schedule = assignments  # Save partial schedule
                self.calculate_scheduling_stats(lecture_requirements, assignments)
                self.progress_label.config(text="Partial schedule - Some lectures unscheduled")
                self.on_view_change()
                self.display_schedule()
                self.update_status()
                
                # Show warning with details
                unscheduled_count = len(self.unscheduled_lectures)
                messagebox.showwarning("Partial Schedule", 
                    f"Generated {len(assignments)} classes.\n"
                    f"{unscheduled_count} lectures could not be scheduled.\n"
                    f"Check the info panel at the bottom of each view for details.")
        
        except Exception as e:
            self.progress_label.config(text="Error occurred")
            messagebox.showerror("Error", f"Schedule generation failed: {str(e)}")
    
    def calculate_scheduling_stats(self, requirements, assignments):
        """Calculate statistics about scheduled and unscheduled lectures"""
        # Track which requirements were scheduled
        scheduled_ids = set()
        for a in assignments:
            # Create a unique ID for each requirement
            req_id = f"{a['course']}|{a['subject']}|{a['type']}"
            scheduled_ids.add(req_id)
        
        # Find unscheduled lectures
        self.unscheduled_lectures = []
        requirement_counts = {}
        
        for req in requirements:
            req_id = f"{req['course']}|{req['subject']}|{req['type']}"
            requirement_counts[req_id] = requirement_counts.get(req_id, 0) + 1
        
        # Count how many of each requirement type were scheduled
        scheduled_counts = {}
        for a in assignments:
            req_id = f"{a['course']}|{a['subject']}|{a['type']}"
            scheduled_counts[req_id] = scheduled_counts.get(req_id, 0) + 1
        
        # Find unscheduled
        for req_id, needed in requirement_counts.items():
            scheduled = scheduled_counts.get(req_id, 0)
            if scheduled < needed:
                parts = req_id.split('|')
                self.unscheduled_lectures.append({
                    'course': parts[0],
                    'subject': parts[1],
                    'type': parts[2],
                    'missing_count': needed - scheduled
                })
        ...
        for teacher in self.teachers:
            ...
            assigned_hours = sum(
                a.get('duration', 1) for a in assignments if a['teacher'] == teacher_name
            )
            teacher_hours[teacher_name] = {
                'assigned': assigned_hours,
                'remaining': 20 - assigned_hours,
        ...
        for course in self.courses:
            ...
            for subject in subjects:
                ...
                if is_lab:
                    expected_hours += 4  # 2-hour lab + 2 lectures
                else:
                    expected_hours += 3  # 3 lectures
            
            # Calculate scheduled hours
            scheduled_hours = sum(
                a.get('duration', 1) for a in assignments if a['course'] == course_name
            )
            
            course_hours[course_name] = {
                'expected': expected_hours,
                'scheduled': scheduled_hours,
                'missing': expected_hours - scheduled_hours
            }
        ...
        self.scheduling_stats = {
            ...
            'total_unscheduled': len(requirements) - len(assignments)
        }
    ...
    def calculate_assignment_score(self, lecture, teacher, day, time_idx, assignments):
        ...
        # Add break quality score
        break_score = self.calculate_break_quality_score(lecture, day, time_idx, assignments)
        score += break_score
    ...
    def calculate_break_quality_score(self, lecture, day, time_idx, assignments):
        ...
        for brk in breaks:
            ...
            # BAD: Only 1 lecture before or after break
                ...
                score -= 30  # Penalty for poor break placement
    ...
    def display_schedule(self):
        ...
        self.display_info_panel(view)
        
        self.timetable_frame.update_idletasks()
        self.canvas.configure(scrollregion=self.canvas.bbox("all"))
    
    def display_info_panel(self, view):
        """Display information panel with scheduling statistics"""
        if not self.scheduling_stats:
            return
        
        # Get row position after the main schedule
        current_row = len(self.time_slots) + 2 if view != "master" else len(self.schedule) + 2
        
        # Create separator
        separator = tk.Frame(self.timetable_frame, bg="#34495e", height=3)
        separator.grid(row=current_row, column=0, columnspan=7, sticky="ew", pady=10)
        current_row += 1
        
        # Info panel frame
        info_frame = tk.Frame(self.timetable_frame, bg="#ecf0f1", relief=tk.RAISED, bd=2)
        info_frame.grid(row=current_row, column=0, columnspan=7, sticky="ew", padx=10, pady=10)
        
        if view == "master":
            self.display_master_info(info_frame)
        elif view == "course":
            self.display_course_info(info_frame)
        elif view == "teacher":
            self.display_teacher_info(info_frame)
        elif view == "classroom":
            self.display_classroom_info(info_frame)
    
    def display_master_info(self, parent):
        """Display overall statistics for master view"""
        tk.Label(parent, text="Scheduling Statistics", font=("Arial", 12, "bold"),
                bg="#ecf0f1", fg="#2c3e50").pack(anchor="w", padx=15, pady=10)
        
        stats = self.scheduling_stats
        
        # Overall stats
        info_text = f"Total Lectures Required: {stats['total_requirements']}\n"
        info_text += f"Successfully Scheduled: {stats['total_scheduled']}\n"
        info_text += f"Unscheduled: {stats['total_unscheduled']}\n\n"
        
        # Unscheduled lectures
        if self.unscheduled_lectures:
            info_text += "UNSCHEDULED LECTURES:\n"
            for unsch in self.unscheduled_lectures:
                info_text += f"  • {unsch['course']} - {unsch['subject']} ({unsch['type'].upper()}): {unsch['missing_count']} lecture(s)\n"
            info_text += "\n"
        else:
            info_text += "All lectures successfully scheduled!\n\n"
        
        # Teacher workload summary
        info_text += "TEACHER WORKLOAD:\n"
        for teacher_name, data in sorted(stats['teacher_hours'].items()):
            info_text += f"  • {teacher_name}: {data['assigned']}h assigned, {data['remaining']}h available\n"
        
        text_widget = tk.Text(parent, height=20, font=("Courier", 9), bg="white",fg="#2c3e50",
                             relief=tk.FLAT, wrap=tk.WORD)
        text_widget.pack(fill=tk.BOTH, expand=True, padx=15, pady=(0, 15))
        text_widget.insert(1.0, info_text)
        text_widget.config(state=tk.DISABLED)
    
    def display_course_info(self, parent):
        """Display statistics for course view"""
        course_name = self.selected_entity.get()
        if not course_name:
            return
        
        tk.Label(parent, text=f"Course: {course_name}", font=("Arial", 12, "bold"),
                bg="#ecf0f1", fg="#2c3e50").pack(anchor="w", padx=15, pady=10)
        
        stats = self.scheduling_stats
        course_stats = stats['course_hours'].get(course_name, {})
        
        info_text = f"Expected Hours: {course_stats.get('expected', 0)}h\n"
        info_text += f"Scheduled Hours: {course_stats.get('scheduled', 0)}h\n"
        info_text += f"Missing Hours: {course_stats.get('missing', 0)}h\n\n"
        
        # Find unscheduled lectures for this course
        course_unscheduled = [u for u in self.unscheduled_lectures if u['course'] == course_name]
        
        if course_unscheduled:
            info_text += "UNSCHEDULED LECTURES:\n"
            for unsch in course_unscheduled:
                info_text += f"  • {unsch['subject']} ({unsch['type'].upper()}): {unsch['missing_count']} lecture(s)\n"
        else:
            info_text += "All lectures for this course are scheduled!\n"
        
        # Show teachers teaching this course
        info_text += "\nTEACHERS:\n"
        course_teachers = set(a['teacher'] for a in self.schedule if a['course'] == course_name)
        for teacher in sorted(course_teachers):
            teacher_data = stats['teacher_hours'].get(teacher, {})
            teacher_course_hours = sum(
                a.get('duration', 1) for a in self.schedule 
                if a['teacher'] == teacher and a['course'] == course_name
            )
            info_text += f"  • {teacher}: {teacher_course_hours}h for this course "
            info_text += f"({teacher_data.get('assigned', 0)}h total, {teacher_data.get('remaining', 0)}h available)\n"
        
        text_widget = tk.Text(parent, height=12, font=("Courier", 9), bg="white",fg="#2c3e50",
                             relief=tk.FLAT, wrap=tk.WORD)
        text_widget.pack(fill=tk.BOTH, expand=True, padx=15, pady=(0, 15))
        text_widget.insert(1.0, info_text)
        text_widget.config(state=tk.DISABLED)
    
    def display_teacher_info(self, parent):
        """Display statistics for teacher view"""
        teacher_name = self.selected_entity.get()
        if not teacher_name:
            return
        
        tk.Label(parent, text=f"Teacher: {teacher_name}", font=("Arial", 12, "bold"),
                bg="#ecf0f1", fg="#2c3e50").pack(anchor="w", padx=15, pady=10)
        
        stats = self.scheduling_stats
        teacher_data = stats['teacher_hours'].get(teacher_name, {})
        
        info_text = f"Assigned Hours: {teacher_data.get('assigned', 0)}h / 20h\n"
        info_text += f"Remaining Capacity: {teacher_data.get('remaining', 0)}h\n"
        info_text += f"Subjects: {teacher_data.get('subjects', 'N/A')}\n\n"
        
        # Show workload by course
        teacher_courses = {}
        for a in self.schedule:
            if a['teacher'] == teacher_name:
                course = a['course']
                teacher_courses[course] = teacher_courses.get(course, 0) + a.get('duration', 1)
        
        if teacher_courses:
            info_text += "TEACHING LOAD BY COURSE:\n"
            for course, hours in sorted(teacher_courses.items()):
                info_text += f"  • {course}: {hours}h\n"
        
        # Check for unscheduled lectures this teacher could teach
        info_text += "\nSUBJECTS WITH UNSCHEDULED LECTURES:\n"
        teacher_subjects = [s.strip() for s in teacher_data.get('subjects', '').split(',')]
        has_unscheduled = False
        
        for unsch in self.unscheduled_lectures:
            subject_clean = unsch['subject'].replace(' (Lab)', '')
            if subject_clean in teacher_subjects:
                has_unscheduled = True
                info_text += f"  • {unsch['course']} - {unsch['subject']}: {unsch['missing_count']} lecture(s)\n"
        
        if not has_unscheduled:
            info_text += "  None - All lectures scheduled!\n"
    ...
    def display_classroom_info(self, parent):
        """Display statistics for classroom view"""
        classroom_name = self.selected_entity.get()
        if not classroom_name:
            return
        
        tk.Label(parent, text=f"Classroom: {classroom_name}", font=("Arial", 12, "bold"),
                bg="#ecf0f1", fg="#2c3e50").pack(anchor="w", padx=15, pady=10)
        
        # Calculate utilization
        classroom_classes = [a for a in self.schedule if a['classroom'] == classroom_name]
        total_hours = sum(a.get('duration', 1) for a in classroom_classes)
        
        # Calculate utilization percentage (out of 60 possible hours: 10 slots x 6 days)
        max_hours = len(self.time_slots) * len(self.days)
        utilization = (total_hours / max_hours) * 100 if max_hours > 0 else 0
        
        info_text = f"Total Classes: {len(classroom_classes)}\n"
        info_text += f"Total Hours Used: {total_hours}h\n"
        info_text += f"Utilization: {utilization:.1f}% ({total_hours}/{max_hours} available hours)\n\n"
        
        # Show courses using this classroom
        classroom_courses = {}
        for a in classroom_classes:
            course = a['course']
            classroom_courses[course] = classroom_courses.get(course, 0) + 1
        
        if classroom_courses:
            info_text += "COURSES USING THIS ROOM:\n"
            for course, count in sorted(classroom_courses.items(), key=lambda x: x[1], reverse=True):
                info_text += f"  • {course}: {count} class(es)\n"
